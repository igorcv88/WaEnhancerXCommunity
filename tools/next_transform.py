...
menu_cases = '''            case R.id.menuitem_test_theme -> getTextareaContentAsync().thenAccept(content ->
                    runOnUiThread(() -> {
...
if 'case R.id.menuitem_test_theme' not in text:
    text = text.replace('            case R.id.menuitem_exit -> finish();\n',
                        menu_cases + '            case R.id.menuitem_exit -> finish();\n')
...
if 'private void notifyCssChanged()' not in text:
    text = text.replace('    private void exportAsZip(Uri uri) {\n',
                        helpers + '    private void exportAsZip(Uri uri) {\n')
if 'LocalDiagnostics.record(this, "css", "Validated CSS saved")' not in text:
...
text = read(path)
start = text.index('    public static Properties getProperties(SharedPreferences prefs, String key, String checkKey) {')
end = text.index('    public static int tryParseInt(', start)
replacement = '''    public static Properties getProperties(SharedPreferences prefs, String key, String checkKey) {
...
if 'LocalDiagnostics.record(this, "lifecycle", "Manager process started")' not in text:
